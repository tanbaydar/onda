...
def _replace(schema_editor, *, cascade):
    ...
    for table, replacement in TABLES:
        ...
        matches = [name for name, details in constraints.items() if details["columns"] == ["user_id"] and details["foreign_key"] and details["foreign_key"][0].lower() == "onda_user"]
        if len(matches) != 1:
        ...
        schema_editor.execute(
            ...
            f"REFERENCES {quote('ONDA_USER')} ({quote('id')}){suffix}"
        )
