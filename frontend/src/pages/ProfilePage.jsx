...
function ProfileStatistics({ username, version = 0 }) {
  ...
  return <section className="profile-statistics"><h2 className="section-heading">Statistics</h2>{state.error ? <div className="action-feedback" role="alert"><p>Statistics could not be refreshed.</p><button className="recovery-action quiet-control" type="button" onClick={() => setRetry((value) => value + 1)}>Retry</button></div> : null}<div className="profile-statistics-strip"><div className="profile-stat stat-lead"><span className="stat-value">{statistics.events_in_been}</span><span className="stat-label">Events in Been</span></div><div className="profile-stat stat-reviews"><span className="stat-value">{statistics.written_reviews}</span><span className="stat-label">Written reviews</span></div><div className="profile-stat stat-venues"><span className="stat-value">{statistics.venues_visited}</span><span className="stat-label">Venues visited</span></div><div className="profile-stat stat-cities"><span className="stat-value">{statistics.cities_visited}</span><span className="stat-label">Cities visited</span></div><div className={`profile-judgment-unit${statistics.average_rating_given.state === "available" ? " is-available" : ""}`}><div className="profile-stat"><span className="stat-value">{statistics.average_rating_given.state === "available" ? statistics.average_rating_given.value.toFixed(1) : "—"}</span><span className="stat-label">Average rating given</span></div><div className={`profile-histogram-group${ratingCount === 0 ? " is-empty" : ""}`}><RatingHistogram className="profile-stat-histogram" buckets={ratingBuckets} staticLabel={ratingCount === 0 ? "Rating distribution: no ratings given yet." : null} />{ratingNote ? <p className="profile-histogram-note">{ratingNote}</p> : null}</div></div></div></section>;
}
    ...
    <main className="profile-page">
      ...
      <header className="profile-header">
        ...
        <div className="profile-identity-copy">
          <h1 className="identity-title">{profile.display_name}</h1>
          ...
          {owner ? <Link className="profile-edit-link mobile-target" to="/settings/profile">Edit profile</Link> : <><FollowControl relationship={data.relationship} pending={followPending} onChange={changeFollow} />{followError ? <div className="profile-follow-error" role="alert"><p>{followError}</p><button className="recovery-action quiet-control" type="button" disabled={followPending} onClick={retryFollow}>{followPending ? "Retrying…" : "Retry"}</button></div> : null}</>}
