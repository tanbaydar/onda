...
export default function SearchPage() {
  ...
  return (
    <main className="search-page" onKeyDown={onKeyDown}>
      ...
      <nav className="search-scopes" aria-label="Search scope">{SCOPES.map(([value, label]) => <button key={value} className={scope === value ? "active" : ""} type="button" onClick={() => { if (trimmed) commitRecent(); setScope(value); setActiveIndex(-1); }}>{label}</button>)}</nav>
      ...
      {state.data && resultCount ? <SearchResults data={state.data} scope={scope} activeIndex={activeIndex} onActiveIndex={setActiveIndex} onResultOpen={commitRecent} onViewAll={(nextScope) => { commitRecent(); setScope(nextScope); }} /> : null}
