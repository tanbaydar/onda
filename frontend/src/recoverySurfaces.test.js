...
test("Activity loads notifications independently from mark-all-read bookkeeping", () => {
  const source = read("./pages/ActivityPage.jsx");

  assert.doesNotMatch(source, /Promise\.all/);
  assert.ok(source.indexOf('fetchJson("/api/me/notifications/"') < source.indexOf("markAllRead(controller.signal)"));
  assert.ok(source.indexOf("results: data.results") < source.indexOf("markAllRead(controller.signal)"));
  assert.match(source, /Activity is visible, but it could not be marked as read\./);
  assert.match(source, /Retry marking as read/);
});

test("Profile follow failures stay local to the action and preserve profile data", () => {
  const source = read("./pages/ProfilePage.jsx");
  const changeFollow = section(source, "async function changeFollow", "  if (state.loading)");

  assert.match(changeFollow, /setFollowState\(\{ saving: false, error \}\)/);
  assert.doesNotMatch(changeFollow, /setState\(/);
  assert.match(source, /Follow status could not be changed\./);
  assert.match(source, /className="profile-action-error" role="alert"/);
});
