...
| Discover Events title | 24px | 30px | Stable Discover page identity |
...
| Event detail | Identity hierarchy changes by past/upcoming state. Every event reserves the governed flyer/placeholder slot. Past identities show date without start time; upcoming identities retain date and time. Full Lineup group. Rating/WBT page data precedes one unlabelled owner block. Your Circle/Public/attendee sections share the aligned content column. |
| Profile Been/Reviews | Centered Instagram-derived identity spacing, inline followers/following, full-measure follow/edit affordance, Statistics, tab hairline, Letterboxd-derived diary rows, and Favorites. Diary rows place 80×100 flyer, title/year, venue, judgment/Been date, then optional review prose/likes. Private stub keeps public identity counts but withholds private modules. |
...
| Activity `/activity` | H1, an independently loaded follow-request disclosure when requests are pending, then one 800px notification ledger. Whole notification rows route. Unread names 600; read state recedes to muted/500. Relative timestamp. A matching pending follow-request notification repeats Approve/Delete so the request is actionable from both Instagram-style entry points; resolved and unrelated rows have no action chrome. |
...
- Discover uses the ruled "Discover Events" header with city selector, tabs, 80×100 rows, rating gate for Recent, and sentinel continuation.
