# Deploy Onda to one AWS EC2 instance

This runbook deploys Onda to an ARM64 Ubuntu `t4g.small` with Docker
...
- `ONDA_DOMAIN`: the real application hostname, such as `onda.example.com`.
...
This runbook uses Onda names for a fresh deployment. When upgrading an existing
checkout created before the rename, copy each domain and database value into its
corresponding `ONDA_*` environment key before switching releases. Keep the existing
database name, database user, bucket, IAM role, host path, and Compose volumes for
that first deploy. This repository change does not rename external infrastructure;
those are separate operator-controlled cutovers.
...
7. Name it `expire-onda-backups`, apply it to all objects, acknowledge that
...
**Management** that `expire-onda-backups` is enabled with a 30-day expiration.
...
2. Choose **Next**, name it `OndaBackupWriter`, and create it.
3. Open **IAM → Roles → Create role → AWS service → EC2**.
4. Attach `OndaBackupWriter`, name the role `OndaEC2Role`, and create it.
...
2. Name it `onda-web`, select the instance VPC, and add inbound rules:
...
6. Under network settings, select the `onda-web` security group. Ensure the
   instance receives a public IPv4 address, or associate an Elastic IP after
   launch so DNS does not change after a stop/start.
7. Under **Advanced details → IAM instance profile**, select `OndaEC2Role`.
...
- **Security → IAM role** is `OndaEC2Role`.
...
The final command should identify the attached `OndaEC2Role`; no access keys
...
```sh
cd /home/ubuntu
git clone YOUR_REPOSITORY_URL onda
cd /home/ubuntu/onda
git switch main
cp .env.example .env
chmod 600 .env
...
`ONDA_DB_PASSWORD`, and `ONDA_DB_ROOT_PASSWORD`, respectively. Set:

```dotenv
ONDA_DOMAIN=ONDA_DOMAIN
...
DJANGO_DEFAULT_FROM_EMAIL=noreply@ONDA_DOMAIN
DJANGO_EMAIL_HOST=
...
ONDA_DB_NAME=onda
ONDA_DB_USER=onda
ONDA_DB_PASSWORD=GENERATED_APP_DATABASE_PASSWORD
ONDA_DB_ROOT_PASSWORD=GENERATED_DISTINCT_ROOT_PASSWORD
ONDA_DB_HOST=db
ONDA_DB_PORT=3306
BACKUP_BUCKET=s3://YOUR_BUCKET_NAME/onda
...
```sh
stat -c '%a %n' .env
grep -E '^(ONDA_DOMAIN|ONDA_DB_NAME|ONDA_DB_USER|BACKUP_BUCKET|AWS_REGION)=' .env
...
DJANGO_DEFAULT_FROM_EMAIL=Onda <noreply@YOUR_VERIFIED_DOMAIN>
...
curl -I -H 'Host: ONDA_DOMAIN' http://127.0.0.1
```
...
record**—create an `A` record for `ONDA_DOMAIN` pointing to the instance's
...
```sh
curl -fsSIL https://ONDA_DOMAIN
curl -fsSI https://ONDA_DOMAIN/api/cities/
```
...
Open `https://ONDA_DOMAIN` in a private browser window. Verify Discover loads,
...
latest_dump=$(find backups -type f -name 'onda-*.sql.gz' -print | sort | tail -1)
gzip -t "$latest_dump"
gzip -dc "$latest_dump" | sed -n '1,20p'
./infrastructure/scripts/verify-backup.sh "$latest_dump"
```
...
**S3 → YOUR_BUCKET_NAME → onda** in the AWS console; the instance cannot read
or list the bucket.
...
and installs `/etc/logrotate.d/onda`. It is safe to rerun after deployment.
...
```cron
15 2 * * * /usr/bin/flock -n /var/lock/onda/backup.lock /home/ubuntu/onda/infrastructure/scripts/backup.sh >> /home/ubuntu/onda/logs/backup.log 2>&1
15 3 * * * /usr/bin/flock -n /var/lock/onda/sync-ra.lock /usr/bin/docker compose --project-directory /home/ubuntu/onda exec -T web python manage.py sync_ra >> /home/ubuntu/onda/logs/sync-ra.log 2>&1
*/5 * * * * /usr/bin/flock -n /var/lock/onda/healthcheck.lock /home/ubuntu/onda/infrastructure/scripts/production-healthcheck.sh >> /home/ubuntu/onda/logs/healthcheck.log 2>&1
30 4 1 * * /usr/bin/flock -n /var/lock/onda/restore-check.lock /home/ubuntu/onda/infrastructure/scripts/verify-backup.sh >> /home/ubuntu/onda/logs/restore-check.log 2>&1
```
...
```sh
crontab -l
/usr/bin/flock -n /var/lock/onda/backup.lock /home/ubuntu/onda/infrastructure/scripts/backup.sh
/usr/bin/flock -n /var/lock/onda/sync-ra.lock /usr/bin/docker compose --project-directory /home/ubuntu/onda exec -T web python manage.py sync_ra
/usr/bin/flock -n /var/lock/onda/healthcheck.lock /home/ubuntu/onda/infrastructure/scripts/production-healthcheck.sh
/usr/bin/flock -n /var/lock/onda/restore-check.lock /home/ubuntu/onda/infrastructure/scripts/verify-backup.sh
tail -n 100 /home/ubuntu/onda/logs/backup.log
tail -n 100 /home/ubuntu/onda/logs/sync-ra.log
tail -n 100 /home/ubuntu/onda/logs/healthcheck.log
tail -n 100 /home/ubuntu/onda/logs/restore-check.log
```
...
7. Name the alarm `OndaMonthlyEstimatedCharges`.
...
`infrastructure/scripts/deploy.sh` performs a fast-forward-only pull, rebuilds
images, starts services, runs migrations, and prunes unused images. It never
removes volumes. Do not run
