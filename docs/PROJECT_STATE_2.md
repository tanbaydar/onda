...
> Branch: `main`
>
> Verified committed HEAD before this document was created: `c841c80`
>
> Remote state at that checkpoint: `main` and `origin/main` aligned
>
> Purpose: provide a self-contained, evidence-based handoff for the current
> ingestion, catalog, identity, and Been implementation.

## Status of this document

This document supersedes `docs/PROJECT_STATE.md` for current implementation and
runtime status. The original file remains useful as a detailed history of Milestone
1, but its top-level statements are now stale: the catalog API, React frontend,
custom user model, session authentication, and first diary/rating slice all exist.
...
3. Session-based identity plus the first private user-data feature: Been entries
   with ratings.
...
- follows and private follow requests;
- favorites;
- onboarding flow;
- written reviews;
- review likes;
- rating-distribution display;
- Your Circle aggregates or review sections;
- Will Be There;
- feeds;
- notifications;
...
11. `docs/PROJECT_STATE.md`
    - Historical Milestone 1 handoff; stale for current product status.
12. `docs/archive/`
...
The verified pushed checkpoint before creating this document was:
...
This new document itself is expected to make the tree dirty until committed.
...
- frontend production build: passed, 28 transformed modules;
...
### Privacy architecture has only its first consumer

`visible_to` and `for_aggregation` are established, but profiles, circles, feeds,
reviews, and follows do not yet exist. Every future attributed entry endpoint must
use the visibility boundary rather than inline filtering.
...
### Review warning repayment

Removing a rating currently warns that attendance remains. When reviews exist, Q187–
188 require the warning and mutation to cover review/like deletion consequences.
...
No new slice is authorized merely by this handoff.

The next planning conversation should begin by reading the relevant product answers
and deciding the smallest consuming slice. Likely nearby work includes rating
distribution and written reviews, but the exact boundary must be approved before
models or UI are added.

Before any public-profile/review/feed slice:

- preserve self-only email;
- route attributed diary/review data through an owned visibility boundary;
- keep private contributions anonymous in aggregates;
- decide response shapes before implementation;
- write genuine red contract tests;
- mutation-check false-positive-prone 404/privacy branches.
