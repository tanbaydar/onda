...
- **The core user is a LOGGER, not a reader.** They build a complete diary to flex their event history ("danced 23 times"). Reviews are optional decoration on the log, not the point.
- **The diary unit is a "dance."** You don't log nights — you log dances. Stats say "Danced," not "Attended."
...
- Stat row — four derived counts, computed on read, hidden until first logged dance, count exactly what the log shows (never disagree), only grow, dead numbers in MVP:
  - **Danced** — distinct events with any log (set rating, vibe rating, stars-only all count)
...
- Working title ("Danced" used in design briefs) — name not final.
