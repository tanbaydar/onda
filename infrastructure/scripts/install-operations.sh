...
lock_dir="/var/lock/onda"

health_entry="*/5 * * * * /usr/bin/flock -n $lock_dir/healthcheck.lock $script_dir/production-healthcheck.sh >> $logs_dir/healthcheck.log 2>&1"
restore_entry="30 4 1 * * /usr/bin/flock -n $lock_dir/restore-check.lock $script_dir/verify-backup.sh >> $logs_dir/restore-check.log 2>&1"
...
sed "s|__LOGS_DIR__|$logs_dir|g; s|__LOG_OWNER__|$(id -un)|g; s|__LOG_GROUP__|$(id -gn)|g" \
  "$repo_dir/infrastructure/operations/onda.logrotate" >"$logrotate_config"
sudo install -m 0644 "$logrotate_config" /etc/logrotate.d/onda
sudo logrotate --debug /etc/logrotate.d/onda >/dev/null

echo "Installed health and restore schedules without replacing existing cron entries."
echo "Installed host log rotation at /etc/logrotate.d/onda."
